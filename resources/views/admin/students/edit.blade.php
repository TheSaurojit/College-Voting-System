...
@section('content')
    <div class="max-w-2xl" x-data="{ programType: '{{ old('program_type', $student->semester ? 'Degree' : ($student->class ? '+2' : '')) }}' }">
        {{-- Back Link --}}
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Update the student information below.</p>
            </div>

            <form method="POST" action="{{ route('admin.students.update', $student) }}" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $student->name) }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="Enter student's full name">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="tel" id="phone" name="phone" value="{{ old('phone', $student->phone) }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="Enter phone number">
                    @error('phone')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative cursor-pointer">
                            <input type="radio" name="program_type" value="+2" x-model="programType" class="peer sr-only" {{ old('program_type', $student->class ? '+2' : '') == '+2' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="program_type" value="Degree" x-model="programType" class="peer sr-only" {{ old('program_type', $student->semester ? 'Degree' : '') == 'Degree' ? 'checked' : '' }}>
                            <div class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-gray-200 bg-white text-gray-600 transition-all duration-200 peer-checked:bg-indigo-50 peer-checked:border-indigo-500 peer-checked:text-indigo-700 hover:bg-gray-50">
                ...
                <div x-show="programType === '+2'" x-transition x-cloak>
                    ...
                    <select id="class" name="class" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                        ...
                        <option value="11" {{ old('class', $student->class) == '11' ? 'selected' : '' }}>Class 11</option>
                        <option value="12" {{ old('class', $student->class) == '12' ? 'selected' : '' }}>Class 12</option>
                    </select>
                    @error('class')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="roll_no" name="roll_no" value="{{ old('roll_no', $student->roll_no) }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200" placeholder="Enter roll number">
                    @error('roll_no')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
